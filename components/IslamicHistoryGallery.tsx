...
import { ISLAMIC_HISTORY_ELEMENTS } from '../constants';

interface IslamicHistoryGalleryProps {
  onSelect: (query: string) => void;
  isLoading?: boolean;
}

const IslamicHistoryGallery: React.FC<IslamicHistoryGalleryProps> = ({ onSelect, isLoading }) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-14">
      {ISLAMIC_HISTORY_ELEMENTS.map((element, idx) => (
        <button
          key={element.id}
        ...
        >
          ...
          <div className="w-full aspect-[16/10] overflow-hidden relative">
            ...
            {/* Historical Tag */}
            <div className="absolute top-6 left-6 px-4 py-1.5 bg-bd-red/90 backdrop-blur-md rounded-full text-[9px] font-black text-white uppercase tracking-[0.3em]">
              ইসলামি ইতিহাস
            </div>
          </div>

          {/* Content Part */}
          <div className="p-8 md:p-12 flex flex-col flex-grow bg-white dark:bg-gray-900/40 backdrop-blur-sm relative">
            {/* Decorative element */}
            ...
            <h4 className="text-2xl md:text-3xl font-black text-bd-green dark:text-bd-green font-noto mb-4 group-hover:text-bd-red transition-colors">
              {element.title}
            </h4>
...
export default IslamicHistoryGallery;
