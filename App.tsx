...
const App: React.FC = () => {
  ...
  const handleSearch = async (searchQuery?: string) => {
    ...
    setError(null);
    setStreamingText('তথ্য অনুসন্ধান করা হচ্ছে...');
    setResult(null);
    setIsMenuOpen(false);

    // Scroll to the results area immediately
    setTimeout(() => {
    ...
    try {
      ...
      setError('তথ্য খুঁজে পেতে সমস্যা হচ্ছে। দয়া করে আপনার ইন্টারনেট কানেকশন চেক করুন এবং পুনরায় চেষ্টা করুন।');
  ...
  return (
    <div className="min-h-screen flex flex-col transition-colors duration-300">
      ...
      {/* Side Menu */}
      <div className={`fixed inset-0 z-[250] transition-opacity duration-300 ${isMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setIsMenuOpen(false)}></div>
        <div className={`absolute top-0 right-0 h-full w-[85%] max-w-[400px] bg-white dark:bg-[#0a0a0a] shadow-2xl transition-transform duration-500 ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col pt-32 pb-12 px-8`}>
          {navItems.map((item, i) => (
            <button key={i} onClick={() => scrollToSection(item.id)} className="text-3xl font-black text-bd-green dark:text-bd-green font-noto mb-8 text-left hover:text-bd-red transition-all">
              {item.label}
            </button>
          ))}
        </div>
      ...
      <main id="main-content">
        <section id="home" className="relative min-h-[90vh] flex flex-col items-center justify-center pt-32 pb-20 overflow-hidden">
          ...
          <div className="container mx-auto px-6 text-center z-10">
            ...
            <h1 className="text-6xl md:text-8xl lg:text-[135px] font-black text-bd-green dark:text-bd-green mb-8 leading-[1.1] md:leading-[1] font-noto tracking-tighter">
              শেকড়ের <span className="text-bd-red italic">সন্ধানে</span> <br /> 
              <span className="gradient-text">দেশজ্ঞানের ময়দানে</span>
            </h1>
            <p className="text-lg md:text-3xl themed-text-muted mb-12 md:mb-20 max-w-5xl mx-auto font-medium leading-relaxed font-noto">ইতিহাস, ঐতিহ্য, পর্যটন থেকে শুরু করে ডিজিটাল বাংলাদেশ — সবকিছুই এখন আপনার হাতের মুঠোয়।</p>
            ...
            <div className="max-w-5xl mx-auto relative group">
              ...
              <div className="relative themed-card p-2 md:p-4 rounded-[30px] md:rounded-[60px] shadow-2xl overflow-hidden flex flex-row items-stretch border-2 border-bd-green/20 group-hover:border-bd-green/40 transition-all duration-500 backdrop-blur-3xl">
                  ...
                  <button onClick={() => handleSearch()} className="bg-bd-green text-white px-5 md:px-20 py-3 md:py-6 rounded-[20px] md:rounded-[50px] font-black uppercase tracking-widest hover:bg-bd-red transition-all duration-500 shadow-xl text-sm md:text-2xl flex items-center justify-center gap-2 md:gap-5 active:scale-95 group/btn shrink-0">
                    <span>খুঁজুন</span>
                  </button>
              </div>
            </div>
        ...
        <section id="tourist-spots" className="py-20 md:py-32 bg-white dark:bg-black/20">
          <div className="container mx-auto px-6">
            ...
            <h3 className="text-3xl md:text-7xl font-black text-bd-green font-noto mb-16">সেরা পর্যটন কেন্দ্রসমূহ</h3>
               ...
               {TOURIST_LANDMARKS.map((spot) => (
                 <button key={spot.id} onClick={() => handleSearch(spot.query)} className="group flex flex-col themed-card rounded-[40px] md:rounded-[60px] border-2 shadow-lg hover:shadow-2xl transition-all overflow-hidden h-full text-left">
                    ...
                    <div className="p-8 md:p-10">
                      <h4 className="text-2xl md:text-3xl font-black text-bd-green font-noto mb-4">{spot.title}</h4>
                      <p className="themed-text-muted mb-6">{spot.description}</p>
                      <span className="text-bd-green font-black uppercase text-[10px] tracking-widest flex items-center gap-3">বিস্তারিত দেখুন →</span>
                    </div>
                 </button>
               ))}
            </div>
        ...
        <section id="history" className="py-20 md:py-32 bg-gray-50 dark:bg-black/40">
          <div className="container mx-auto px-6">
            ...
            <h3 className="text-3xl md:text-6xl font-black text-bd-green font-noto mb-12">বাংলার হাজার বছরের পথচলা</h3>
        ...
        <section id="directory" className="py-20 md:py-32">
          <div className="container mx-auto px-6 text-center">
            ...
            <h3 className="text-2xl md:text-7xl font-black text-bd-green font-noto mb-16">বাংলার অফুরন্ত জ্ঞানভাণ্ডার</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {EXPLORE_CATEGORIES.map((cat) => (
                <button key={cat.id} onClick={() => handleSearch(cat.query)} className="themed-card group p-8 rounded-[40px] border border-transparent hover:border-bd-green/20 hover:shadow-2xl transition-all text-left flex flex-col items-start gap-6">
                  <div className={`w-14 h-14 ${cat.color} rounded-2xl flex items-center justify-center text-2xl shadow-xl group-hover:scale-110 transition-all text-white`}>{cat.icon}</div>
                  <div><h4 className="text-xl font-black text-bd-green font-noto mb-2">{cat.title}</h4><p className="themed-text-muted text-xs leading-relaxed">{cat.description}</p></div>
                </button>
              ))}
            </div>
          </div>
      ...
      <footer className="bg-white dark:bg-[#050505] border-t-2 border-gray-100 dark:border-gray-900 py-16">
        <div className="container mx-auto px-6 text-center">
          <div className="flex items-center gap-4 mb-8 justify-center">
            <div className="w-12 h-12 bg-bd-green rounded-2xl flex items-center justify-center text-white font-black text-xl">🇧🇩</div>
            <span className="text-3xl font-black text-bd-green font-noto">দেশজ্ঞান</span>
          </div>
          <p className="themed-text-muted mb-8">&copy; {new Date().getFullYear()} DESHGYAN ENCYCLOPEDIA. BUILT WITH AI PRIDE.</p>
        </div>
      </footer>
    </div>
  );
